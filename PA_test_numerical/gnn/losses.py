...
def polynomial_loss(Bs, noisevar, Gw=True):
    ...
    def loss(H_batch_train, y_pred):
        ...
        numerical = True
        if numerical:
            print(f'training numericallyyy----------------------------------------------------------------')
            Ce = compute_ce_numerical(W, G, Bs)
        else:
...
def compute_ce_numerical(W, G, Bs):
    ...
    # amplify
    b1 = tf.cast(Bs[0], tf.complex64)
    b3 = tf.cast(Bs[1], tf.complex64)
    y = b1 * x + b3 * x * tf.cast(tf.math.abs(x)**2, tf.complex64)

    # compute distortion
    e = y - G @ x

    # compute covariance of distortion
    cove = (1/nr) * e @ tf.transpose(e, conjugate=True, perm=(0, 2, 1))
    return cove
