...
if __name__ == '__main__':
    ...
    # model params
    layers = 8#[8]  # [3, 8, 16]
    features = 128  #[128] # [32, 64, 128, 256, 512]
    activation = 'lrelu'
    skips = None #['sum', learned_per_layer', 'mlp', 'gnn', None]  # , 'mlp', 'gnn', 'learned_per_layer']
    # folder to store results
    folder_for_test = f'stored_models_pa_numerical_test'
    aggregation_string = 'mean'

    # start training
    print(f'----------------------OUTER LOOP GNN k {K} , m {M}---------------------')
    sim_params['Pt'] = M  # m changes => total transmit power changes P_t = M
    noise_var_set = sim_params['Pt'] / (
                10 ** (snr_tx_set / 10))  # update noise vars with new Pt
    print(f"Pt: {sim_params['Pt']} - snr set: {10 * np.log10(sim_params['Pt'] / noise_var_set)}")

    # train at 1 SNR point only 20 dB
    nvar = noise_var_set[0]
    print(f'training at an IBO of {ibo[4]} with PA: {different_backoffs[4]} - third order model')

    # update the simulation params
    sim_params['Bs'] = different_backoffs[4]
    sim_params['K'] = K
    sim_params['M'] = M
    ...
    training_params['dl'] = features
    training_params['activation'] = activation
    training_params['skips'] = skips
    training_params['aggregation'] = aggregation_string
    training_params['note'] = f'{ibo[4]}_IBO_{layers}' \
                              f'_layers_{features}_features'
