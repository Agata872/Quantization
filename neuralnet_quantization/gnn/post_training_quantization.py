...
def representative_data_gen(Htest):
    #convert channel to correct shape for the nn
    Htest_real = C2R(Htest)
    Htest_flat = tf.reshape(Htest_real, [Htest.shape[0], -1, 2])
    bs = 100
    for i in range(int(Htest_flat.shape[0]/bs)):
        yield Htest_flat[i*bs:(i+1)*bs, :, :]

if __name__ == '__main__':
    #define path of the model to be tested
    path = r'C:\Users\Thomas\OneDrive - KU Leuven\Documents\GitHub\Quantization\neuralnet_quantization\gnn\models\M_64_K_2_nvar_0.64\gnn_mean_aggregation_skips_None_BO_-3_8_layers_128_features_lreluM_64_K_2'
    ...
    #todo quantize the weights of the GNN

    #quantize the inputs to the GNN (fake quantization still fp32 but quantized to see the effect)
    #convert channel to correct shape for the nn
    Htest_real = C2R(Htest).astype(np.float32)
    Htest_flat = tf.reshape(Htest_real, [Htest.shape[0], -1, 2])
    minvalue = np.min(Htest_flat)
    maxvalue = np.max(Htest_flat)
    print(f'min, max: {minvalue, maxvalue}')
    bits = 3
    Htest_flat_quant = tf.quantization.fake_quant_with_min_max_args(Htest_flat, minvalue, maxvalue, num_bits=bits)
    #todo print quantization levels as check:

    #sanity check
    if minvalue <= 0 <= maxvalue:#adjustment that TF function makes
        print('in if')
        scale = (maxvalue - minvalue) / (2 ** bits - 1)
        min_adj = scale * np.round(minvalue / scale)
        max_adj = maxvalue + min_adj - minvalue
    nrlevels = 2**bits
    range = max_adj - min_adj
    delta = range / (nrlevels-1)
    levels = min_adj + np.arange(nrlevels) * delta
    print(f'{levels=}')
    sortedvalues = np.sort(np.array(Htest_flat_quant).flatten())
    plt.scatter(np.arange(2*128*10000), sortedvalues)
    for level in levels:
        plt.axhline(y=level, linestyle='--')
    plt.show()

    #problem each tensor operation needs to be quantized based on its input range
    #which is variable for all of them so you would need to run a few batches to get mins and maxes for each range
    #try to change pwr normalization layer so that we can use TF lite maybe try a model trained for 1 epoch with
    #pwr normalization layer removed to see if this will quantize
    ...
    evaluate(sim_params, Htest[0:10, :, :], model, path=os.path.join(path, f"dumy_testing{sim_params['pa']}"))
