...
def get_efficient_GNN(M, K, feature_size, Pt, layers, activation='lrelu', batchnorm=False, skip_connection=False,
                      aggregation='sum'):
...
def get_GNN(M, K, feature_size, Pt, layers, activation='lrelu', batchnorm=False, skip_connection=False,
            aggregation='sum'):
    input_feature_size = 2
...
def train(training_params, sim_params, mainfolder='stored_models'):
    ...
    if layertype == 'gnn':
        model = get_GNN(M, K, feature_size, Pt, layers, activation, batchnorm=batchnorm,
...
if __name__ == '__main__':
    ...
    folder_for_test = f'stored_models_post_quant_friendly'
                    ...
                    for m in range(len(Ms)):
                        for skip in skips:
                            ...
                            for nvar in noise_var_set:
                                ...
                                training_params[
                                    'note'] = f'{channel_model}_{ibo[4]}_IBO_{layer_type}_{aggregation_string}_aggregation_skips_{skip}_{l}' \
