...
def Rsum_loopy(H, W, diag_alpha, diag_beta, noisevar, Pt):
    """
    :param H: M x K channel matrix
    :param W: M x K precoding matrix
    :return: sum rate
    """
    #todo wrong system model, adjust H^H => H^T
    ...
    for k in range(K):
        ...
        # usefull sig
        sig = Pt * np.abs(hk.T.conj() @ diag_alpha @ wk)
        ...
        for kprime in range(K):
            ...
            wkprime = W[:, kprime]
            if kprime != k:
                interference += Pt * np.abs(hkprime.T.conj() @ diag_alpha @ wkprime)**2

        # dist
        Rqq = diag_alpha @ diag_beta @ np.diag(np.diag(Pt * W @ W.conj().T))
        wwh = W @ W.conj().T
        abswwh = np.abs(wwh)
...
def get_sumrate(b, H, Pt, noise_vars, path=None):
    if b == 'inf':
        beta = 0
    else:
    ...
    for i in range(H.shape[0]):
        ...
        if H.shape[-1] == 1:
            W = MRT_precoding(H[i, :, :], 1).conj()
        else:
            W = ZF_precoding(H[i, :, :], 1).conj()
        pwr_test = np.linalg.norm(W, ord='fro') ** 2


        # compute sum rate
        Rsum_all[i, :] = Rsum_loopy(H[i, :, :], W, diag_alpha, diag_beta, noise_vars, Pt)
        print('check')
...
if __name__ == '__main__':
    ...
    M, K = 64, 1#nr antennas and users
    Pt = M
