...
from MIMO_sims.Rsum_all import Rsum_Bussgang_Rx, Rsum_Bussgang_Rx_per_channel
...
if __name__ == '__main__':
    ...
    #r'D:\thomas.feys\Quantization\precoding_quantization\non_lin_precoding\stored_models_v2\M_32_K_4_bs_128_layers_4_dl_128_tau_1\1_bits_GNN_gumbel_softmax_hard_2024-07-25_13-06-46'
    ...
    output_dir = os.path.join(model_dir, 'post_training_cdfs')
    ...
    # set GPU or CPU
    device = torch.device("cpu")#torch.device("cuda" if torch.cuda.is_available() else "cpu")  # check if GPU is available
    ...
    Rsum_batches = np.zeros((nr_batches, batch_size, len(snr_points)))
    Rsum_batches_zf = np.zeros((nr_batches, batch_size, len(snr_points)))
    Rsum_batches_zf_agc = np.zeros((nr_batches, batch_size, len(snr_points)))
    Rsum_batches_zf_noquant = np.zeros((nr_batches, batch_size, len(snr_points)))
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(test_dataloader):
            ...
            # compute sumrate
            Rsum_batches[i, :, :] = Rsum_Bussgang_Rx_per_channel(H.cpu().numpy(), snr_points, bits=bits, quant='non-uniform',
            ...
            # zf/mrt benchmark
            Rsum_batches_zf[i, :, :] = Rsum_Bussgang_Rx_per_channel(H.cpu().numpy(), snr_points, bits=bits, quant='non-uniform',
            ...
            # zf/mrt benchmark
            Rsum_batches_zf_agc[i, :, :] = Rsum_Bussgang_Rx_per_channel(H.cpu().numpy(), snr_points, bits=bits, quant='non-uniform',
                                                         Pt=M, automatic_gain_control=True, precoding='zf-mrt',
                                                         quant_params_path=quant_params_path,
                                                         s_provided=s.cpu().numpy(),
                                                         normalize_across_symbols=True)
            # zf/mrt no quant
            Rsum_batches_zf_noquant[i, :, :] = Rsum_Bussgang_Rx_per_channel(H.cpu().numpy(), snr_points, bits=bits, quant='none',
                                                             Pt=M, precoding='zf-mrt', s_provided=s.cpu().numpy(),
                                                             normalize_across_symbols=True)



    # reshape to nr_channels x nr_snr_points
    Rsum = Rsum_batches.reshape(-1, Rsum_batches.shape[-1])
    Rsum_zf = Rsum_batches_zf.reshape(-1, Rsum_batches.shape[-1])
    Rsum_zf_agc = Rsum_batches_zf_agc.reshape(-1, Rsum_batches.shape[-1])
    Rsum_zf_noquant = Rsum_batches_zf_noquant.reshape(-1, Rsum_batches.shape[-1])


    # set snr point
    snr_tx = 20
    snr_idx = np.where(snr_points==snr_tx)[0][0]

    # compute ecdf for given snr point
    Rsum_gnn_cdf = ECDF(np.real(Rsum[:, snr_idx]))
    Rsum_zf_cdf = ECDF(np.real(Rsum_zf[:, snr_idx]))
    Rsum_zf_agc_cdf = ECDF(np.real(Rsum_zf_agc[:, snr_idx]))
    Rsum_zf_noquant_cdf = ECDF(np.real(Rsum_zf_noquant[:, snr_idx]))

    # plot
    plt.plot(Rsum_gnn_cdf.x[1:], Rsum_gnn_cdf.y[1:], label='GNN')
    plt.plot(Rsum_zf_cdf.x[1:], Rsum_zf_cdf.y[1:], label='ZF')
    plt.plot(Rsum_zf_agc_cdf.x[1:], Rsum_zf_agc_cdf.y[1:], label='ZF + AGC')
    plt.plot(Rsum_zf_noquant_cdf.x[1:], Rsum_zf_noquant_cdf.y[1:], label='ZF no quant')
    plt.title(f'snr test: {snr_tx} - M: {M} - K: {K} - B: {bits}')
    plt.ylabel('CDF')
    plt.xlabel('sum rate [Bits/channel use]')
    plt.legend()
    fig = plt.gcf()
    tikzplotlib_fix_ncols(fig)
    tikzplotlib.clean_figure()
    tikzplotlib.save(os.path.join(output_dir, 'cdf.tex'))
    fig.savefig(os.path.join(output_dir, 'cdf.pdf'))
    plt.show()
